...
SHOT_MATCH_PX = 8  # match threshold on warped image pixel coords (legacy/default)
# Mode-specific pixel difference thresholds for shot deduplication
RIFLE_SHOT_MATCH_PX = 16   # Rifle: larger threshold due to warped hole appearance
PISTOL_SHOT_MATCH_PX = 8   # Pistol: standard threshold
CENTER_EMA_ALPHA = 0.3
_shot_ledgers: dict[str, dict[str, Any]] = {}
_shot_ledgers_lock = threading.Lock()


def _get_shot_match_px() -> float:
    """Get the shot match pixel threshold based on current shooting mode."""
    global shooting_mode
    if shooting_mode == 'rifle':
        return RIFLE_SHOT_MATCH_PX
    else:
        return PISTOL_SHOT_MATCH_PX
...
def _merge_detected_shots(user_key: str,detected: list[dict[str, Any]],force_new_shot: bool = False) -> dict[str, Any]:
    ...
    # 🔒 Track true hole growth only
        ...
        if r < MIN_NEW_HOLE_RADIUS_PX:
             if not _overlaps_any_detected(x, y, r, detected, _get_shot_match_px()):
              continue
...
def api_rifle():
    ...
    # Ensure PI socket is connected so vibration events can auto-trigger score update.
    if selected_ip and not pi_socket.connected:
        connect_pi_socket(selected_ip)
