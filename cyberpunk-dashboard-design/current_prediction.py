...
RIFLE_HOLE_THRESHOLD = 50  # Threshold value for detecting dark hole pixels (0-255)
...
def _rifle_decimal_score_all_rings(d_norm_edge: float, outer_radius_px: float, center_distance_px: float, pellet_radius_px: float, mode: str = "rifle") -> float:
    """Calculate decimal rifle score for ANY ring (not just 10-point).
    
    Uses 10 discrete bands within each ring:
        Ring 10: 10.0 - 10.9
        Ring 9: 9.0 - 9.9
        Ring 8: 8.0 - 8.9
        etc.
    ...
    Args:
        ...
        outer_radius_px: Outer target radius in pixels
        center_distance_px: Absolute center-to-center distance in pixels
        pellet_radius_px: Pellet radius in pixels
        mode: 'rifle' or 'pistol'
    
    Returns:
        Decimal score (10.9, 10.2, 9.8, 8.3, etc.)
    """
    if mode == "pistol":
        ring_ratios = PISTOL_RING_RATIOS
    else:
        ring_ratios = RIFLE_RING_RATIOS
    
    # Determine base ring (integer score)
    base_score = 0
    for ring_num in range(10, 0, -1):
        if d_norm_edge < ring_ratios[ring_num]:
            base_score = ring_num
            break
    ...
    # Get this ring's inner and outer boundaries
    ring_outer_ratio = ring_ratios[base_score]
    ring_inner_ratio = ring_ratios[base_score - 1] if base_score > 1 else 1.0
    
    # Ring width in normalized space
    ring_width_norm = ring_inner_ratio - ring_outer_ratio
    if ring_width_norm <= 0:
        return float(base_score)
    
    # How far into this ring is the edge? (0.0 = outer boundary, 1.0 = inner boundary)
    progress_in_ring = (ring_inner_ratio - d_norm_edge) / ring_width_norm
    progress_in_ring = max(0.0, min(1.0, progress_in_ring))
    
    # Convert to 0.0-0.9 decimal range
    decimal_part = progress_in_ring * 0.9
    final_score = round(float(base_score) + decimal_part, 1)
    
    # DEBUG: Log ring calculation details
    if base_score in [8, 9, 10]:  # Only log for interesting rings
        print(f"[DecimalScore] Ring {base_score}: outer={ring_outer_ratio:.5f}, inner={ring_inner_ratio:.5f}, "
              f"d_norm={d_norm_edge:.5f}, progress={progress_in_ring:.3f}, decimal={decimal_part:.2f}, final={final_score}", flush=True)
...
def _annotate_and_score(frame, shooting_mode="rifle"):
    ...
    # Lowered confidence from 0.12 to 0.05 to catch faint rifle holes, relying on _validate_hole_shape
    # to filter out any false positives strictly within those bounded boxes.
    res = model(infer_frame, conf=0.08)[0]
    names = res.names
    ...
    for hx, hy, hr in hole_detections:
        ...
        # Relaxed for rifles since bullet tears paper irregularly
        min_circ = 0.45 if mode == "rifle" else 0.55
        if _validate_hole_shape(frame_gray, hx, hy, hr, min_circularity=min_circ, min_aspect_ratio=0.5):
    ...
    if not validated_holes and hole_detections:
        if DEBUG_DETECTIONS:
            print(
    ...
    for hx, hy, hr in validated_holes:
        ...
        # For rifle mode: refine hole center using thresholding within black contour
        if mode == "rifle" and RIFLE_HOLE_CENTER_REFINEMENT:
            ...
            hx, hy = _refine_hole_center_rifle(infer_frame_original, hx, hy, hr, black_center_for_refine, black_radius_for_refine)
        ...
        # For rifle mode: use CENTER-to-CENTER distance (NOT inner edge)
        # For pistol mode: use inner edge (center distance minus pellet radius)
        if mode == "rifle":
            # Rifle scoring: use center distance directly (not edge calculation)
            score = _rifle_decimal_score_all_rings(d_norm, outer_radius_px, sqrt(dx*dx + dy*dy), expected_hole_radius_px, mode="rifle")
            # DEBUG: Log normalized distance and score for diagnostics
            print(f"[Rifle Scoring] Shot at ({hx:.1f}, {hy:.1f}): d_norm={d_norm:.4f}, score={score:.1f}", flush=True)
        else:
        ...
        if USE_CONTOUR_10_SCORING and score > CONTOUR_ACTIVATION_MIN_SCORE:
            ...
            if contour_result is not None:
                ...
                if contour_circularity >= CONTOUR_MIN_CIRCULARITY:
                    ...
                    # Use RIFLE_RING_RATIOS[9] which is where ring 9 starts (ring 10 ends)
                    if mode == "rifle":
                        ring10_outer_ratio = RIFLE_RING_RATIOS[9]  # 0.12088 - ring 10 outer boundary
                    else:
                    ...
                    if contour_score is not None:
                        ...
                        if mode == "rifle":
                            d_norm_edge = max(d_norm - hole_radius_ratio, 0.0)
                            score = score_fn(d_norm_edge, d_norm)
                        else:
                            d_norm_edge = max(d_norm - effective_hole_radius_ratio, 0.0)
                            score = score_fn(d_norm_edge)
    ...
    VISUALIZE_CENTER_DISTANCE = True  # Toggle this to show/hide distance visualization
